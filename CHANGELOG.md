...
- 新增单条可编辑状态卡，实时显示当前模型、服务商和真实上下文占用；无 token 数据时隐藏上下文。
- 新增单条可编辑执行过程卡，按真实工具开始、完成、失败事件编号并更新状态。
- 最终回复独立发送，关闭重复 runtime footer、中途助手碎片和周期性长任务提醒。
- 状态卡编辑失败时不补发重复气泡，状态更新使用每轮独立消息 ID 和异步锁。

### 安装与验证

- 新增 `gateway/feishu_realtime_display.py` 安装、备份、回滚和卸载处理。
- 新增 22 条 Hermes v0.20.0 (`1be70d6`) 实时显示替换规则。
- 安装器支持 LF/CRLF 源码匹配；重复安装保持幂等。
- `verify.ps1` 检查实时渲染文件、配置开关和全部显示补丁标记。
- 包检查只扫描 Git 已跟踪和未忽略文件，避免读取本地验证备份；执行环境明确为 PowerShell 7。
